...
\begin{example}
    ...
    \begin{center}
        \begin{tikzpicture}[>=latex',join=bevel,node distance=70bp]
            ...
            \node[fill=white,initial, state] (AA) {AA};
            \node[fill=white,state, right of=AA] (BA) {BA};
            \node[fill=white,state, right of=BA] (CA) {CA};

            % States of the second copy
            \node[fill=white,state] (AB) at (0, -4) {AB};
            \node[fill=white,state, right of=AB] (BB) {BB};
            \node[fill=white,state, right of=BB] (CB) {CB};

            \path (AA) edge[edge0, bend right] node {0} (AB)
                       edge[edge1,bend left=15] node {1} (BB)
                  (BA) edge[edge0, ] node {0} (AA)
                       edge[edge1, ] node {1} (CA)
                  (CA) edge[edge0, bend left] (AA)
                       edge[edge1, loop above] (CA);

            \path (AB) edge[edge0, loop below] (AB)
                       edge[edge1, ] node {1} (BB)
                  (BB) edge[edge0, bend left=15] node {0} (AA)
                       edge[edge1, bend right=15] node {1} (CA)
...
\todo{Add example of the Muller to Parity construction}
...
\todo{Add example of parity to buchi automaton construction}
...
\todo{Apparently, this is not true. $\mu$-calculus is reported to be the fragment
of FOL invariant by bisimilarity (van Benthem theorem).}
...
\todo{Add examples of mu-calculus formulas and their meaning}
...
We currently have a quasipolynomial algorithm due to XXX \todo{add ref},
...
version can be found at \todo{add gitlab url}

\todo{add my python code here}
